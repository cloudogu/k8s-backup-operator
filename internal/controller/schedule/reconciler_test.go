	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.validatorErr != nil {
				requireRecordedEvent(t, recorder, schedule, corev1.EventTypeWarning, invalidScheduleEventReason, "BackupSchedule has an invalid schedule: invalid")
			} else {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.metadataErr != nil {
				requireRecordedEventContains(t, recorder, schedule, corev1.EventTypeWarning, finalizerRemovalFailedEventReason, "Failed to remove finalizer", backupv1.BackupScheduleFinalizer, tt.metadataErr.Error())
			} else {
				requireNoRecordedEvent(t, recorder)
			}
