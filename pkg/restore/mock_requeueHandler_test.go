...
func (_m *mockRequeueHandler) Handle(ctx context.Context, contextMessage string, restore *v1.Restore, originalErr error, requeueStatus string) (reconcile.Result, error) {
	ret := _m.Called(ctx, contextMessage, restore, originalErr, requeueStatus)

	var r0 reconcile.Result
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string, *v1.Restore, error, string) (reconcile.Result, error)); ok {
		return rf(ctx, contextMessage, restore, originalErr, requeueStatus)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string, *v1.Restore, error, string) reconcile.Result); ok {
		r0 = rf(ctx, contextMessage, restore, originalErr, requeueStatus)
	} else {
		r0 = ret.Get(0).(reconcile.Result)
	}

	if rf, ok := ret.Get(1).(func(context.Context, string, *v1.Restore, error, string) error); ok {
...
func (_c *mockRequeueHandler_Handle_Call) Run(run func(ctx context.Context, contextMessage string, restore *v1.Restore, originalErr error, requeueStatus string)) *mockRequeueHandler_Handle_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(string), args[2].(*v1.Restore), args[3].(error), args[4].(string))
	})
	return _c
}
...
func (_c *mockRequeueHandler_Handle_Call) RunAndReturn(run func(context.Context, string, *v1.Restore, error, string) (reconcile.Result, error)) *mockRequeueHandler_Handle_Call {
	_c.Call.Return(run)
	return _c
}
