...
var NewVeleroProvider = func(k8sClient K8sClient, recorder EventRecorder, namespace string) (Provider, error) {
	return velero.NewDefaultProvider(k8sClient, namespace, recorder), nil
}

// Get returns the provider by the given name and calls a function on the provider object to check if it is ready.
func Get(ctx context.Context, object runtime.Object, name k8sv1.Provider, namespace string, recorder EventRecorder, k8sClient K8sClient) (Provider, error) {
	...
	case k8sv1.ProviderVelero:
		provider, err = NewVeleroProvider(k8sClient, recorder, namespace)
		if err != nil {
...
func GetAll(ctx context.Context, namespace string, recorder EventRecorder, k8sClient K8sClient) []Provider {
	...
	for _, providerName := range knownProviders {
		provider, err := Get(ctx, nil, providerName, namespace, recorder, k8sClient)
		if err != nil {
